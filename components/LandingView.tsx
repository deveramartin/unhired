...
import { ChevronRight, FileText, Flame, ShieldAlert, Sparkles } from 'lucide-react';
...
export default function LandingView({ isLoggedIn }: LandingViewProps) {
  ...
  const handleCTA = () => router.push(isLoggedIn ? '/roast' : '/login');
  const handleSecondary = () => router.push(isLoggedIn ? '/history' : '/login');
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-16 w-full flex-grow flex flex-col justify-center">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-stretch">
          ...
          <div className="lg:col-span-7 bg-zinc-900 border border-white/5 rounded-3xl p-6 sm:p-10 flex flex-col justify-center select-none space-y-6 text-center lg:text-left relative overflow-hidden">
            ...
            <div className="inline-flex self-center lg:self-start items-center space-x-2 px-3 py-1 bg-slate-950 border border-white/10 rounded-full">
            ...
            <div className="grid grid-cols-3 gap-4 pt-6 max-w-md mx-auto lg:mx-0 font-mono text-left border-t border-white/5">
              {[
                { value: '482K+', label: 'Egos Destroyed', color: 'text-emerald-400' },
                { value: '2.4m', label: 'Buzzwords Burnt', color: 'text-emerald-200' },
                { value: '99.7%', label: 'Ghost Rate Match', color: 'text-teal-400' },
              ].map(({ value, label, color }) => (
                <div key={label} className="border-l border-emerald-500/30 pl-3">
          ...
          {/* Right: Buzzword Translator Widget */}
          <div className="lg:col-span-5 bg-zinc-900 border border-white/5 rounded-3xl p-6 sm:p-8 flex flex-col justify-between shadow-[0_10px_35px_rgba(0,0,0,0.5)] max-w-md mx-auto w-full space-y-6">
            <div className="flex items-center justify-between pb-3 border-b border-white/5">
              <span className="font-mono text-[10px] text-emerald-400 font-bold tracking-widest uppercase flex items-center gap-1.5">
                ...
                AI Buzzword De-Obfuscator
              </span>
              <span className="font-mono text-[9px] text-slate-500">Status: Fully Judgemental</span>
            </div>

            <div className="space-y-4 flex-1 flex flex-col justify-center">
              <div>
                <span className="font-mono text-[9px] uppercase text-slate-500 block mb-1">What you wrote on your CV:</span>
                <div className="bg-slate-950 border border-white/5 p-3 rounded-xl font-mono text-slate-300 text-xs text-left relative overflow-hidden">
      ...
      <div className="bg-zinc-950 border-t border-white/5 py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { step: '01', title: 'File Feeding Zone', color: 'text-slate-400', desc: "Drag-and-drop your glossy PDF resume. We don't steal your private info—we just extract your dignity." },
              { step: '02', title: 'Annihilation Scan', color: 'text-emerald-400', desc: 'Our parser identifies your grammar sins, inflated job titles, and copy-paste templates with surgical cybernetic eyes.' },
              { step: '03', title: 'Suffer in Audio', color: 'text-teal-400', desc: 'Read the scathing markdown breakdown, then click play to hear the AI recruiter speak your demise aloud via Speech Synthesizer.' },
            ].map(({ step, title, color, desc }) => (
              <div key={step} className="bg-zinc-900 p-6 rounded-3xl border border-white/5 hover:border-emerald-500/20 transition-all text-center space-y-3">
                <div className={`mx-auto w-10 h-10 rounded-full bg-slate-950 border border-white/10 flex items-center justify-center font-mono font-bold text-xs ${color}`}>
                  {step}
                </div>
                <h3 className={`text-xs font-mono font-bold uppercase tracking-wider ${color}`}>{title}</h3>
